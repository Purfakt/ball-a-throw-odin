...
draw_blind_info :: proc(ms: ^MS_GamePlay, ui: UiContext) {
	current_score_text := fmt.ctprintf("Score: %v", ms.current_score)
	blind_score_text := fmt.ctprintf("Blind: %v", ms.blind_score)
	hands_text := fmt.ctprintf("Hands: %v", ms.run_data.hands_per_blind - ms.hands_played)
	discards_text := fmt.ctprintf("Discards: %v", ms.run_data.discard_per_blind - ms.discards_used)
...
draw_sort_buttons :: proc(ms: ^MS_GamePlay, ui: UiContext) {
...
draw_play_discard_buttons :: proc(ms: ^MS_GamePlay, ui: UiContext) {
...
draw_game_over :: proc(ms: ^MS_GamePlay, ui: UiContext) {
	w := i32(ui.w)
	h := i32(ui.h)
...
get_play_button_rect :: proc(ms: ^MS_GamePlay, ui: UiContext) -> rl.Rectangle {
...
get_discard_button_rect :: proc(ms: ^MS_GamePlay, ui: UiContext) -> rl.Rectangle {
	hand_size := i32(len(ms.hand_pile))
	hand_w := (CARD_WIDTH * hand_size) + (CARD_MARGIN * (hand_size - 1))
	start_x := i32(ui.w / 2) - (hand_w / 2)
	end_x := start_x + hand_w
	...
	return {f32(end_x) + 40, button_y - f32(button_h) - 10, f32(button_w), f32(button_h)}
}
...
get_card_hand_target_layout :: proc(
	...
	is_selected := c.handle_array_contains(ms.selected_cards[:], handle)
