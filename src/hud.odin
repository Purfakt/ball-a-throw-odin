...
draw_hud :: proc(ctx: ^GameContext, layout: Layout) {
	...
	if game_play_data != nil {
		selected_consumable_index = game_play_data.selected_consumable_index
	}

	draw_player_cards_bar(ctx, layout, selected_consumable_index)
}

draw_player_cards_bar :: proc(ctx: ^GameContext, layout: Layout, selected_consumable_index: int) {
	...
	for rect, i in consumable_rects {
		...
		if i < len(ctx.run_data.tarot_cards) {
			consumable := ctx.run_data.tarot_cards[i]
			if tarot_data, ok := consumable.(c.TarotData); ok && tarot_data.tarot != .None {
...
get_consumable_slot_rects :: proc(layout: Layout) -> [2]rl.Rectangle {
	bar_area := layout.joker_bar
	card_slot_width := f32(80)
	card_slot_height := f32(110)
	card_padding := f32(10)
	spacing_between_groups := f32(20)
	jokers_width := 5 * card_slot_width + 4 * card_padding
	start_x := bar_area.x + (bar_area.width / 2) - (card_slot_width / 2)
	slot_y := bar_area.y + (bar_area.height / 2) - (card_slot_height / 2)
	consumable_start_x := start_x + jokers_width + spacing_between_groups
	rects: [2]rl.Rectangle
	for i in 0 ..< 2 {
		rects[i] = {
			consumable_start_x + f32(i) * (card_slot_width + card_padding),
			slot_y,
			card_slot_width,
			card_slot_height,
		}
